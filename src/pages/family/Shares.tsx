...
export default function FamilyShares() {
  ...
  const calculateMemberStats = () => {
    ...
    shares.filter(s => s.is_active).forEach(share => {
      ...
      memberShares.set(memberName, (memberShares.get(memberName) || 0) + 1);
    });
    return memberShares;
  };

  const stats = {
    totalShares: shares.filter(s => s.is_active).length,
    totalValue: shares.filter(s => s.is_active).reduce((sum, s) => sum + parseFloat(s.share_value.toString()), 0),
              ...
              {canManageFinances && (
                ...
                <DialogContent>
                  ...
                  <form onSubmit={handleCreateShare} className="space-y-4">
                    ...
                    <div>
                      <Label htmlFor="share_value">Share Value (FCFA)</Label>
                      <Input
                        id="share_value"
                        type="number"
                        min="0"
                    ...
                    <Button type="submit" className="w-full">Issue Share</Button>
                  </form>
                </DialogContent>
              </Dialog>
              )}
                      ...
                      shares.map((share) => (
                        <tr key={share.id} className="border-b hover:bg-muted/50">
                          ...
                          <td className="p-4 text-right font-mono">
                            {parseFloat(share.share_value.toString()).toLocaleString()} FCFA
                          </td>
                  ...
                  <tbody>
                    {Array.from(calculateMemberStats()).map(([memberName, shareCount]) => {
                      const memberTotalValue = shares
                        ...
                        .reduce((sum, s) => sum + parseFloat(s.share_value.toString()), 0);
